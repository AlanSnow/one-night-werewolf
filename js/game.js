...
$(document).ready(function(){
    ...
    console.log(gameText,playerOrder)
    ...
    function voiceListener(){
        if(currentText==0){
            changeVoice(gameText[currentText])//播放第一个语音
            $(".charator").text(charatorNumber[parseInt(playerOrder[currentText])]);
            $("#message").text(msgBox[parseInt(playerOrder[currentText])]);
        }else{
            var tmpCount = setTimeout(function(){changeVoice(gameText[currentText])},17000);//17s后播放下一语音
            timeCount();//15秒开始计时
        }
        //判断结束语音
        if(currentText >= gameText.length-1){
    ...
    function changeVoice(str){
        ...
        var voiceUrl = "http://tsn.baidu.com/text2audio?lan=zh&ie=UTF-8&spd=3&text="+zhText;
    ...
    function limitAllActive(num){
        if($(".under .active").length >= num ) {
            $(".under img").addClass("disabled");
        }else{
            $(".under img").removeClass("disabled");
        }
    }

    function limitOutActive(num){
        if($(".out .active").length >= num ) {
            $(".out img").addClass("disabled");
        }else{
            $(".out img").removeClass("disabled");
        }
    }

    function limitDeskActive(num){
        if($(".desk .active").length >= num ) {
            $(".desk img").addClass("disabled");
        }else{
            $(".desk img").removeClass("disabled");
        }
    }

    function limitSelfActive(num){
        $(".desk img")[num].addClass("disabled");
    }
    ...
    $(".under").click(function(){
        ...
        var tmp = "_"+parseInt(playerOrder[order]);
        charatorEvent[tmp]();
    })

    var charatorEvent = {
        //守卫
        _0: function doppelganger(order){
                
        },

        _1: function werewolf(order){
                limitAllActive(1);
                //limitOutActive(num);
                //limitDeskActive(num);
                btnUnbind();
                $("#confirm").click(function(){
                    var lists = $(".out>div, .desk>div");
                    var num = lists.index($(".under .active"));
                    eventOrder.push([num,1]);
                });
                $("#reset").click(function(){
                    var lists = $(".out>div,.desk>div");
                    var num = lists.index($(".under .active"));
                    eventOrder.push([num,0]);
                })
            },

        _2: function werewolf(order){
                limitAllActive(1);
                //limitOutActive(num);
                //limitDeskActive(num);
                btnUnbind();
        ...
        _3: function minion(order){
            
            },

        _4: function drunk(order){
            
            },

        _5: function insomniac(order){
                
            },

        _6: function mason(order){
                
            },

        _7: function mason(order){
                
            },

        _8: function robber(order){
                
            },

        _9: function seer(order){
                
            },
        
        _10: function troublemaker(order){
                
            },
        
        _14: function witch(order){
                
            },
        
        _15: function alpha(order){
                
            },
        
        _16: function dream(order){
                
            },

        _17: function mystic(order){
                
            },

        _18: function apprentice(order){
                
            },

        _19: function aura(order){
                
            },

        _20: function curator(order){
                
            },

        _21: function paranormal(order){
                
            },

        _22: function idiot(order){
                
            },

        _23: function revealer(order){
                
            },

        _24: function sentinel(order){
                limitAllActive(1);
                //limitOutActive(num);
                //limitDeskActive(num);
                limitSelfActive(order)
                btnUnbind();
                $("#confirm").click(function(){
                    var lists = $(".out>div, .desk>div");
                    var num = lists.index($(".under .active"));
